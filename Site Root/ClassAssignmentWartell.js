 ...
 * @type {string}
 */
var studentDirectory;
/**
 * \todo make this configuration by application
 */
const visualStudio = "C:\\Program Files (x86)\\Microsoft Visual Studio\\2019\\Community\\Common7\\IDE\\devenv.exe";
/*
...
function itemString(L1, L2, L3) {
    ...
    ret = L1.toString();
    if (L2 !== undefined)
        ret += "." + String.fromCharCode(aCode + L2 - 1);
    if (L3 !== undefined)
        ret += "." + roman_lower(L3);
    return ret;
}
...
export function class_onLoad() {
                ...
                if (h3List !== null && h3List.length !== 0) {
                    ...
                    for (let h3 of h3List) {
                        ...
                        if (h3NoInstructions && instructions.instructions.length != h3InstructionCount) {
                            ...
                            instructions.instructions[h3InstructionCount] = new Instruction(h1c.toString() + "." + h2c.toString() + "." + h3c.toString(), "", h3.innerText.trimStart().slice(0, 10) + " ...", Category.SECTION, section.dataset.pointFraction !== undefined ? parseInt(section.dataset.pointFraction) : 0);
    ...
    document.querySelector("#StudentDirectory").addEventListener('change', (e) => {
        console.log(e);
        studentDirectory = e.target.value; //files[0].match(/(.*)[\/\\]/)[1] || '';
